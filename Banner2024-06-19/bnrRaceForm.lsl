...
// ============================== Upd 2024-08-01 ============================
// Configuration
string m_csVersion = "bnrRaceForm (2024-08-01)";
...
string sGetRoster(key keyDlg, string sLeague, integer iDiv, integer iHeatMax)
{
    ...
    if (sLeague == "Bloodbath")
    {
        ...
        integer iGate = 0;
        integer iMax1B = 99;
        integer iMax1BA = iMax1;
        if (iMax1 < 8)
        {
            iMax1B = 8;
            iMax1BA = iNRacers;
        }
        DebugTrace("iMax1/2/3/B " + (string)iMax1 + "/" +
                   (string)iMax1 + "/" + (string)iMax1 + "/" + (string)iMax1B);
        for (iEntry = 0; iEntry < iNRacers; ++iEntry)
        {

            if (iEntry == iMax1BA) // We are 1 over max1
            {
                // Split list in chat
                sRoster += sResults;
